...
class pdf_mod extends RR_Model {
	...
	public function __construct() {
        ...
        if(empty($this->params['proyecto'])) {
            redirect(base_url('module/load/m/proyecto/a/listado'));
        } else {
           $this->load->model('proyecto_mod','Proyecto');
           $this->proyecto_id  = $this->params['proyecto'];
        }
        $this->breadcrumb->addCrumb('Proyectos',base_url('module/load/m/proyecto/a/listado'));
        $this->breadcrumb->addCrumb($this->Proyecto->getProyectoName($this->proyecto_id),'');
        $this->breadcrumb->addCrumb($this->module_title,base_url('module/load/m/'.$this->module.'/a/listado/proyecto/'.$this->proyecto_id));
    }
    ...
    public function newa(){
        ...
        $data_panel['back']        = base_url('module/load/m/'.$this->module.'/a/listado/proyecto/'.$this->proyecto_id);
        if(!empty($this->id)) {
        ...
        #VALIDO FORM POR PHP
         ...
         if($this->form_validation->run('PDFS')==FALSE){
            ...
            if($query){
                ...
                $data    = array('success' =>$success,'responseType'=>$responseType, 'value'=>base_url('module/load/m/'.$this->module.'/a/listado/proyecto/'.$this->proyecto_id));
            }
